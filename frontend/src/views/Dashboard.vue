    ...
    <section class="alert-section">
      ...
      <div class="alert-table-wrapper">
        ...
        >
          ...
          <el-table-column label="生产日期" width="70">
            <template #default="{ row }">
              <span class="txt-gray">{{ dayjs(row.productionDate).format('YYYY-MM-DD') }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="shelfLife" label="保质期" width="50">
            <template #default="{ row }">
              <span class="txt-gray">{{ row.shelfLife }}天</span>
            </template>
          </el-table-column>
          <el-table-column prop="expiryDate" label="过期日期" width="80">
            <template #default="{ row }">
              <span class="txt">{{ dayjs(row.expiryDate).format('YYYY-MM-DD') }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="remainingDays" label="剩余" width="50" sortable>
            <template #default="{ row }">
